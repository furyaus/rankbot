...
keys = ["Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiJlYjgyODMzMC1mOTEyLTAxMzctNmI4YS0wZmY2ZGZjNmI5ZDAiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNTc1NDk4MzM3LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6Im5hZHVuMTktZ21haWwtIn0.QOlOyg3q-MT_a5s6A0wDNJDOFO-V9nYt-Lcl-X2wTmo", \
"Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI3N2VlMjY0MC0zNGZhLTAxMzgtMzBmNy0xMWRiZTRiMWZlZmUiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNTgyMDg0OTA1LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6InFyeWtwLW9wdGVuLWVtIn0.spXD_EeegWHLeBwUES4o2gtoSvg2IuUv0H6PCX-s9Uo",\
"Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiIxNWU3YThhMC0zNGZiLTAxMzgtZjc0OC0wYmZlODVlOGVlNjQiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNTgyMDg1MTcwLCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6ImZ6c21ob2JlY3NhempqIn0.UKSM-0gFle_sviIzmf2K4jm0efkXxpKlm5Htu9cFo8M",\
"Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI1YjBkYjcyMC0zNGZiLTAxMzgtMzlhZC03NTczNzdiNTc2MzMiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNTgyMDg1Mjg2LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6Iml0bnItdXNlcnBkZi1uIn0.6cPlDyicuWxO7C3v5c1Nn0EBmHv0kTaPJSFfY5vQoy8",\
"Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiIwNGFkNzljMC0zNTE3LTAxMzgtZTQxYy03ZDVjOWM4YTdjZWUiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNTgyMDk3MTY3LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6InNpbHRpemdjcHlldGNjIn0.5r6wNNvBp6wmfHjDFlLOGDvl4rrx0srHWkwHNcdr9vQ"]

header = {
    "Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiJlYjgyODMzMC1mOTEyLTAxMzctNmI4YS0wZmY2ZGZjNmI5ZDAiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNTc1NDk4MzM3LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6Im5hZHVuMTktZ21haWwtIn0.QOlOyg3q-MT_a5s6A0wDNJDOFO-V9nYt-Lcl-X2wTmo",
    "Accept": "application/vnd.api+json"
}
#This JSON file contains the member + pubg player ID
with open("edited_server_list.json", "r") as read_file:
    server_list = json.load(read_file)

client = commands.Bot(command_prefix = '.')
client.remove_command('help')

server_roles = ["Dirt","Spartans","Monsters","Titans","DemiGods","GODS"]
...
async def help(ctx):
    ...
    help_msg = discord.Embed(
        ...
        description="Rank GOD Bot calculates in-game KD and it manages the roles within this discord!"
    )
    ...
    help_msg.add_field(name="enlist:", value=".enlist <USERNAME> , links your discord username with your PUBG in-game name, for example \".enlist Mazun_19\". You only have to do this once!", inline=False)
...
async def checkstats(ctx, Username):
    ...
    if initial_r.status_code != 200:
        ...
        if len(recent_matches) == 0:
            await ctx.send('You haven\'t played a Squad FPP game in the last 14 days so you will be demoted to DIRT, either ask admin to enlist a different account or play some games to get your proper rank!!')
        else:
            if season_losses == 0:
                    tru_KD = total_kills
            else:
                ...
                if games_played < 50:
                    await ctx.send('You have played less than 100 games this season, so you only get DIRT role')
                    ...
                    if ADR < 225:
                        ...
                        await ctx.send(f'{Username}\'s Role = Dirt')
                    elif ADR < 350:  
                        await ctx.send(f'{Username}\'s Role = Spartans')
                    elif ADR < 400:
                        await ctx.send(f'{Username}\'s Role = Monsters')
                    elif ADR < 450:
                        await ctx.send(f'{Username}\'s Role = Titans')
                    elif ADR < 500:
                        await ctx.send(f'{Username}\'s Role = DemiGods')
                    else:
                        await ctx.send(f'{Username}\'s Role = GODS')
            ...
            if season_losses == 0:
                ...
                if games_played < 50:
                    ...
                    if ADR < 250:
                        ...
                        await ctx.send(f'{str(user)[:-5]}\'s Role = Dirt')
                    elif ADR < 350:  
                        await ctx.send(f'{str(user)[:-5]}\'s Role = Spartans')
                    elif ADR < 400:
                        await ctx.send(f'{str(user)[:-5]}\'s Role = Monsters')
                    elif ADR < 450:
                        await ctx.send(f'{str(user)[:-5]}\'s Role = Titans')
                    elif ADR < 500:
                        await ctx.send(f'{str(user)[:-5]}\'s Role = DemiGods')
                    else:
                        await ctx.send(f'{str(user)[:-5]}\'s Role = GODS')
        ...
        if initial_r.status_code != 200:  
            ...
            if ADR < 250:
                ...
                if games_played < 100:
                    ...
                    role = discord.utils.get(ctx.guild.roles, name=new_role)
                    await user.add_roles(role)
                    #Also assign DIRT role to newcomers
                    if new_role != 'Dirt':
                        role = discord.utils.get(ctx.guild.roles, name='Dirt')
                        await user.add_roles(role)
                    #write the updated server list
                    with open("edited_server_list.json", "w") as data_file:
                        json.dump(server_list, data_file, indent=2)
...
async def updateEveryone(ctx):
    ...
    for user in server_list:
        ...
        if no_of_players == 10 or total_additions == len(server_list):
            ...
            for i in range(0,len(season_info['data'])):
                ...
                #Check if the user hasn't played a game in the last 14 days and also if they have a higher rank than DIRT
                if (len(recent_matches) == 0) and (curr_role != 'Dirt'):
                    ...
                    server_list[curr_user]['Rank'] = 'Dirt'
                    await member.remove_roles(role)
    ...
    if len(inactive_users) > 0:
        await channel.send('Following users haven''t played a Squad FPP game in the last 14 days so you will be demoted to DIRT, either ask admin to enlist a different account or play some games to get your proper rank!!')
...
async def get_zeus(ctx):
    ...
    for user in server_list:
        if (server_list[user]['KD'] > maxKD) and (server_list[user]['Rank'] != "Dirt"):
...
async def get_aries(ctx):
    ...
    if current_aries == 'None':
        ...
        while i > -(total_length - 1):
            ...
            if new_server_list[i]['Rank'] != "Dirt":
...
async def reset_everyone(ctx):
    ...
    try:
        for user in server_list:
            ...
            if server_list[user]['Rank'] != "Dirt":
                curr_rank = server_list[user]['Rank']
                server_list[user]['Rank'] = "Dirt"
                role = discord.utils.get(ctx.guild.roles, name=curr_rank)
                member = discord.utils.get(id.members, id=int(user))
                await member.remove_roles(role)
...
client.run('NjUxOTAxMjQzMTU3MTE4OTc3.Xkyj_Q.Oul6nwRhqhcH23Q0N3zc3u8Ei00')
